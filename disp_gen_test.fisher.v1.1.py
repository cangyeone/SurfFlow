...
def invert_one_sample(
    ...
    lam_smooth1_vp=1e-4,
    lam_smooth1_vs=1e-4,
    lam_smooth1_rho=1e-4,
    lam_smooth2_vp=8e-4,
    lam_smooth2_vs=8e-4,
    lam_smooth2_rho=8e-4,
    fisher_damping=1e-4,
    device="cpu",
    print_every=100,
):
    """
    同时对 Vp / Vs / rho 做梯度反演，并加入：
      - 数据项
      - 初始模型先验约束
      - 一阶平滑
      - 二阶平滑
    ...
    Fisher / 局部后验采用:
      H_post ≈ J^T J + H_reg + damping * I

    返回里保留 std_post_vs，方便沿用你现在的 Vs 作图逻辑。
    """
    model.eval()
    ...
    depth_init = x_init_4h[0].detach().clone()
    vp_init = x_init_4h[1].detach().clone()
    vs_init = x_init_4h[2].detach().clone()
    rho_init = x_init_4h[3].detach().clone()

    # 用 softplus 保证正值
    z_vp = inverse_softplus(vp_init).detach().clone().requires_grad_(True)
    z_vs = inverse_softplus(vs_init).detach().clone().requires_grad_(True)
    z_rho = inverse_softplus(rho_init).detach().clone().requires_grad_(True)
    ...
    for it in range(n_iter):
        ...
        vp = torch.nn.functional.softplus(z_vp) + 1e-4
        vs = torch.nn.functional.softplus(z_vs) + 1e-4
        rho = torch.nn.functional.softplus(z_rho) + 1e-4

        x_cur = make_forward_model_input_full(x_init_4h, vp, vs, rho)  # [1,4,H]
        ...
        # -------- prior-to-initial --------
        prior_vp = torch.mean((vp - vp_init) ** 2)
        prior_vs = torch.mean((vs - vs_init) ** 2)
        prior_rho = torch.mean((rho - rho_init) ** 2)
        ...
        # -------- smoothness --------
        s1_vp, s2_vp = smoothness_losses_1d(vp)
        s1_vs, s2_vs = smoothness_losses_1d(vs)
        s1_rho, s2_rho = smoothness_losses_1d(rho)
    ...
    with torch.no_grad():
        # initial prediction
        x_init = make_forward_model_input_full(x_init_4h, vp_init, vs_init, rho_init)
        pred_init, _ = model(x_init, periods=periods_1_nt)

        # final prediction
        vp_inv = torch.nn.functional.softplus(z_vp) + 1e-4
        vs_inv = torch.nn.functional.softplus(z_vs) + 1e-4
        rho_inv = torch.nn.functional.softplus(z_rho) + 1e-4

        x_inv = make_forward_model_input_full(x_init_4h, vp_inv, vs_inv, rho_inv)
        pred_final, _ = model(x_inv, periods=periods_1_nt)

    # -------- Fisher / posterior Hessian approximation --------
    # q = [vp, vs, rho], size = 3H
    J, q0 = compute_jacobian_wrt_model_full_single(model, x_inv, periods_1_nt)  # [2*NT,3H]
    F = J.T @ J                                                                  # [3H,3H]

    Hreg_vp = build_regularization_hessian_1d(
        H,
    ...
    Hreg_vs = build_regularization_hessian_1d(
        H,
    ...
    Hreg_rho = build_regularization_hessian_1d(
        H,
    ...
    Hreg = torch.zeros((3 * H, 3 * H), device=F.device, dtype=F.dtype)
    Hreg[0:H, 0:H] = Hreg_vp
    Hreg[H:2*H, H:2*H] = Hreg_vs
    Hreg[2*H:3*H, 2*H:3*H] = Hreg_rho

    Hpost = F + Hreg + fisher_damping * torch.eye(3 * H, device=F.device, dtype=F.dtype)
    C_post = torch.linalg.inv(Hpost)

    # 只提取 Vs 部分的后验标准差，方便你现有图继续使用
    C_vs = C_post[H:2*H, H:2*H]
    std_post_vs = torch.sqrt(torch.clamp(torch.diag(C_vs), min=0.0))

    return {
        "vp_init": vp_init.detach().cpu(),
        "vs_init": vs_init.detach().cpu(),
        "rho_init": rho_init.detach().cpu(),
        ...
        "J": J.detach().cpu(),          # [2*NT, 3H]
        "F": F.detach().cpu(),          # [3H, 3H]
        "Hpost": Hpost.detach().cpu(),  # [3H, 3H]
...
def plot_inversion_results_4_with_dispersion(
    ...
    for i, res in enumerate(results):
        ...
        lower = vs_inv - 2.0 * std * 0.001
        upper = vs_inv + 2.0 * std * 0.001
...
def plot_inversion_results_8(
    depth_km,               # [H]
    results,                # list of dict
    out_path,
    title_prefix="Gradient inversion",
):
    """
    8 个样本，2x4 布局。
    每个 panel 画:
    ...
    """
    n = len(results)
    assert n == 8, "Please provide exactly 8 inversion results."
    ...
    fig.suptitle(title_prefix, y=0.98, fontsize=14)
    fig.subplots_adjust(bottom=0.12, top=0.92, wspace=0.18, hspace=0.22)
    fig.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def run_inversion_test(
    ...
    for sid in target_ids:
        print(f"\n=== Inverting sample {sid} ===")
        x_true, disp_true, mask_true = dataset[sid]

        periods_1_nt = disp_true[0].unsqueeze(0).to(device)   # [1,NT]
        d_obs_1_2_nt = disp_true[1:3].unsqueeze(0).to(device) # [1,2,NT]

        out = invert_one_sample(
            model=model,
            x_init_4h=x_mean.clone(),
            d_obs_1_2_nt=d_obs_1_2_nt,
            periods_1_nt=periods_1_nt,
            n_iter=1000,
            lr=0.002,
            lam_prior_vp=1e-2,
            lam_prior_vs=2e-3,
            ...
            device=device,
            print_every=max(1, n_iter // 6),
        )

        out["sample_id"] = sid
        out["vs_true"] = x_true[2].detach().cpu()
        out["periods"] = disp_true[0].detach().cpu()       # [NT]
        out["d_obs"] = disp_true[1:3].detach().cpu()       # [2,NT]
        out["final_loss"] = float(out["loss_hist"][-1])
        out["vp_true"] = x_true[1].detach().cpu()
        out["vs_true"] = x_true[2].detach().cpu()
        out["rho_true"] = x_true[3].detach().cpu()
        results.append(out)
