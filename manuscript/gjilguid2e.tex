...
\begin{summary}
Surface-wave dispersion inversion is non-unique: many 1-D elastic structures can explain similar phase-velocity curves. A useful inverse method should therefore return not only a preferred model, but also uncertainty that can be checked. Bayesian inversions and earlier probabilistic neural networks already address this point, but a learned posterior is interpretable only for the prior, forward solver, data mask, noise assumption and parameterisation used to train it. We develop a probabilistic direct-inversion (DI) method in which a depth-controlled conditional rectified flow generates ensembles of 1-D $V_P$, $V_S$ and density profiles from incomplete Rayleigh/Love phase-velocity curves. The aim is to test how such learned posteriors can be audited.

To isolate the role of the training prior, we train two matched samplers from scratch with identical budgets: DI-Strong, a narrow-prior sampler, and DI-Weak, a broad-prior sampler. For models drawn from the strong training prior, DI-Strong gives posterior-median $V_S$ mean absolute error (MAE) of 0.086 km s$^{-1}$ and mean pointwise 16--84 per cent interval coverage of 0.638. When the test structures move towards or outside that prior, DI-Weak gives lower $V_S$ MAE, reducing the near-edge and outside-support errors to 0.171 and 0.499 km s$^{-1}$, compared with 0.372 and 0.782 km s$^{-1}$ for DI-Strong. On a same-site CSRM benchmark using Rayleigh fundamental-mode phase velocities at 8--60 s, DI-Strong gives mean $V_S$ MAE of 0.108 km s$^{-1}$, comparable to QEDisp fundamental-mode multistart inversion (0.104 km s$^{-1}$) and DispFormer phase-only prediction (0.111 km s$^{-1}$). Empirical rescaling of posterior spread improves held-out mean coverage, but it is a calibration diagnostic rather than an observational likelihood. These results support a conservative interpretation: the proposed sampler is competitive as a point estimator, but its main value is that posterior samples, prior support, coverage, missing-band behaviour and noise sensitivity can be checked explicitly.
\end{summary}
...
The central result is that a depth-controlled conditional rectified flow can provide fast posterior ensembles for surface-wave dispersion, but those ensembles must be read together with the training assumptions. In this setting the prior is encoded by the synthetic model generator, velocity and density bounds, depth clipping, layer or control-point parameterisation, wave-type availability, observation masks and the assumed error model. Figure~\ref{fig:workflow} summarizes this idea. We therefore include a matched prior-support test comparing narrow- and broad-prior samplers trained with the same budget, missing-band and noise-sensitivity checks, empirical posterior-spread calibration, and a same-site external benchmark against QEDisp and DispFormer under a common Rayleigh-phase input. The aim is to show how DNN posterior uncertainty can be sampled, compared with established inversions and then audited within a clearly defined synthetic inversion problem.
...
Appendix Table~\ref{tab:priorparameters} lists the main parameter ranges used by the two generators. These values are not tuned to the CSRM benchmark sites or to any field-data example in this manuscript; they define the synthetic inversion problem used to train and test $q_\theta({\bf m}|{\bf d})$.
...
\section{Learned posterior sampler}

\subsection{Conditioning encoder}

The conditional input has five period-indexed channels: normalised period, Rayleigh phase velocity, Love phase velocity, and separate masks for Rayleigh and Love picks. A one-dimensional convolutional encoder with residual blocks maps this input to a 256-dimensional conditioning vector. The encoder sees both measured velocities and the pattern of missing observations, so the posterior sampler can represent cases with Rayleigh only, Love only, both wave types or incomplete period windows. In the present implementation these incomplete observations are represented on a fixed 2--60 s period grid. The masks emulate missing or quality-controlled picks within that grid; they should not be interpreted as unrestricted variable-period generalisation.
...
We also use a simple split-sample calibration diagnostic that rescales posterior spread around the median. For each posterior sample ${\bf m}_s$ and pointwise posterior median $\widetilde{\bf m}$, the scaled sample is
\begin{equation}
{\bf m}^{(\tau)}_s=
\widetilde{\bf m}+\tau({\bf m}_s-\widetilde{\bf m}).
\label{eq:temperature}
\end{equation}
The scalar $\tau$ is fitted on a calibration subset to make mean pointwise 16--84 per cent coverage close to the nominal 0.68 target and is then evaluated on a held-out subset. This operation widens or narrows intervals without changing posterior medians. It is a diagnostic of posterior dispersion, not a replacement for a physical observational-error likelihood. We refer to this operation as posterior-spread calibration; it is analogous to temperature-style calibration in probabilistic machine learning, but here it is implemented as empirical spread rescaling around the posterior median. The production result tables report the fitted spread multipliers and held-out reliability curves for all methods and regimes.

\begin{table*}
\caption{Audit protocol used to interpret the learned posterior samples. The diagnostics are designed to test whether the posterior ensemble is useful under the stated synthetic inversion problem; they are not a claim of calibrated field-data posterior probability.}
\label{tab:auditprotocol}
\centering
\begin{tabular}{@{}p{0.22\textwidth}p{0.35\textwidth}p{0.33\textwidth}@{}}
\toprule
Audit component & Question & Diagnostic used here \\
\midrule
Prior support & Is the target structure represented by the training prior? & Inside-support, near-edge and outside-support tests; pull-in into the strong-prior envelope. \\
Posterior calibration & Do nominal intervals contain synthetic truth at the expected rate? & Pointwise 16--84 per cent coverage, reliability curves, rank/PIT diagnostics and split-sample posterior-spread calibration. \\
Observation completeness & Does uncertainty respond to missing periods? & Fixed-window and random-window missing-band tests on the 2--60 s period grid. \\
Observation noise & How sensitive is the sampler to inconsistent phase velocities? & Evaluation-time additive-noise tests; no additive-noise likelihood is claimed for the main training runs. \\
Physics consistency & Do posterior samples reproduce the conditioning data? & Forward-modelled posterior-predictive dispersion residuals. \\
Bayesian anchor & Is the learned posterior shape compatible with a sampling reference? & Reduced prior-matched MCMC comparison for selected inside-support synthetic cases. \\
External baseline & Is point-estimate performance competitive? & Same-site CSRM benchmark against QEDisp and DispFormer under controlled Rayleigh-phase input. \\
\bottomrule
\end{tabular}
\end{table*}

\subsection{Training-prior support diagnostic}

The prior-support diagnostic changes the test distribution while keeping the forward solver and observation format fixed. Three regimes are used. The inside-support set contains models drawn from the strong-prior generator. The near-edge set contains physically plausible models placed close to the strong-prior limits, for example in Moho depth, sediment thickness, upper-mantle $V_S$ or low-velocity-zone depth and amplitude. The outside-support set remains physically plausible but deliberately exceeds strong-prior support through thicker sediments, shifted Moho depths, stronger low-velocity zones, upper-mantle velocities outside default ranges or sharper gradients.

For compactness, DI-Strong denotes the matched strong-prior sampler, used as a narrow-prior control, and DI-Weak denotes the weak-prior sampler, used as the broad-prior configuration. Because DI-Strong and DI-Weak are trained with matched budgets, the numerical comparison isolates the structural-prior generator; it is a test of training-prior support rather than a universal prior ranking.
...
\subsection{Posterior-sampling performance on synthetic tests}
...
\subsection{Empirical coverage and posterior-spread calibration}

Raw posterior intervals are informative but not automatically calibrated (Fig.~\ref{fig:coverage}). On the inside-support held-out split, raw 68 per cent mean coverage is 0.631 for DI-Strong and 0.580 for DI-Weak. Empirical posterior-spread calibration fitted on a separate calibration half gives held-out mean coverage of 0.681 and 0.685, with spread multipliers of 1.114 and 1.316, respectively. Near-edge and outside-support regimes require much larger corrections for DI-Strong ($\tau=3.160$ and 4.584), reflecting mismatch with the strong training prior. DI-Weak needs smaller near-edge correction ($\tau=1.687$) but still needs substantial outside-support widening ($\tau=3.335$). These results show that the DNN can return a spread of possible models, but that the nominal intervals must be checked before they are read as probabilities.

Depth-binned coverage and rank diagnostics tell the same story with more spatial detail. For inside-support DI-Strong, raw depth-binned 68 per cent coverage ranges from 0.588 to 0.752 across channel-depth bins and becomes 0.641--0.795 after spread calibration. DI-Weak has a wider raw inside-support range, 0.205--0.860, which becomes 0.258--0.941 after calibration; this indicates that one scalar spread multiplier corrects the mean interval width but does not remove all depth- and parameter-dependent miscalibration. Rank/probability-integral-transform (PIT) histograms are also non-uniform before scaling: the two edge bins contain 0.201 of inside-support DI-Strong ranks and 0.281 of inside-support DI-Weak ranks, compared with 0.10 for a uniform diagnostic. We therefore report both mean reliability curves and depth-binned/rank tables rather than relying on one aggregate coverage number.

\begin{figure}
\includegraphics[width=\columnwidth]{figures/fig04_calibration_reliability_v4.pdf}
\caption{Split-sample posterior-spread calibration for the matched production samplers. (a) Raw and spread-calibrated empirical coverage at the nominal 68 per cent interval level. (b) Posterior spread multiplier needed to bring the held-out mean coverage close to 0.68. This calibration changes posterior spread around the median but does not change the posterior median or replace an observational-error model.}
\figalt{Two-panel calibration summary for DI-Strong and DI-Weak across inside-support, near-edge and outside-support regimes, showing raw and spread-calibrated 68 per cent coverage and fitted posterior spread multipliers.}
\label{fig:coverage}
\end{figure}

\subsection{Missing period bands and posterior uncertainty}

The production samplers were trained with intermittent-band mask augmentation to better match incomplete dispersion picks. We evaluate the fair weak-prior checkpoint on 1024 synthetic examples using fixed and random period windows (Appendix Fig.~\ref{fig:missingband}). Reducing the mean valid fraction from 0.351 for the nominal 2--60 s mask to 0.235 for the 2--40 s window increases $V_S$ MAE from 0.164 to 0.192 km s$^{-1}$ and increases mean $V_S$ posterior spread from 0.179 to 0.206 km s$^{-1}$. The narrower 10--30 s window is more restrictive, with valid fraction 0.135, $V_S$ MAE 0.276 km s$^{-1}$ and mean $V_S$ spread 0.287 km s$^{-1}$. The random-window experiment has mean width 29.8 s, valid fraction 0.187, $V_S$ MAE 0.208 km s$^{-1}$ and mean spread 0.221 km s$^{-1}$.
...
The primary synthetic benchmark is mask-only, with no additive phase-velocity noise during training. We therefore treat additive noise as an evaluation-time sensitivity test rather than as a claim of noise-aware training (Appendix Fig.~\ref{fig:noise}). For inside-support DI-Weak, increasing Gaussian phase-velocity noise from 0 to 0.10 km s$^{-1}$ raises $V_S$ MAE from 0.133 to 0.299 km s$^{-1}$, raises mean posterior spread from 0.207 to 0.245, lowers mean raw coverage from 0.577 to 0.405 and increases the 68 per cent spread multiplier from 1.336 to 2.053. DI-Strong shows the same trend, with inside-support $V_S$ MAE increasing from 0.086 to 0.199 km s$^{-1}$ and raw coverage falling from 0.638 to 0.394. This behaviour is expected: a sampler trained on a mask-only benchmark can recognise less consistent inputs through broader posterior summaries, but it has not learned a physical noise likelihood.
...
\subsection{Reduced MCMC anchor for posterior shape}
\label{sec:mcmc_anchor_results}

The preceding diagnostics test coverage and prior support, but they do not by themselves show whether the learned ensemble resembles a conventional sampling posterior. We therefore ran a reduced prior-matched MCMC calculation for six inside-support synthetic cases and compared it with DI-Strong (Appendix Fig.~\ref{fig:mcmcanchorinside}). The MCMC reference uses a lower-dimensional $V_S$ parameterisation and a Gaussian-mixture approximation to the DI-Strong synthetic prior, so it is not an exact posterior for the full procedural generator. It is nevertheless a useful anchor because the forward solver, period grid, mask and error scale are matched explicitly.

Across the six validated inside-support cases, the mean DI--MCMC posterior-median $V_S$ difference is 0.057 km s$^{-1}$, the mean p05--p95 interval overlap is 0.58 and the mean DI/MCMC interval-width ratio is 1.21. Thus, inside the stated prior support, the conditional-flow samples have broadly similar median profiles and interval widths to this reduced Bayesian reference. The same comparison is less stable near and outside the prior support (Appendix Fig.~\ref{fig:mcmcanchortransition}), which is consistent with the prior-support diagnostics above.
...
The mean best-fit Rayleigh dispersion RMS for QEDisp is 0.010 km s$^{-1}$. The mean forward-modelled Rayleigh phase RMS for DispFormer is 0.048 km s$^{-1}$ in the phase-only run and 0.035 km s$^{-1}$ in the phase+group run. For DI, the additional QC diagnostic is reference-profile inclusion: the mean fraction of CSRM reference $V_S$ samples inside the DI p05--p95 band is 0.680 for DI-Strong and 0.856 for DI-Weak. This quantity checks whether the DI interval contains a reference model at these sites; it is not empirical field-posterior coverage.

\begin{figure*}
\includegraphics[width=\textwidth]{figures/fig05_common_input_benchmark.pdf}
\caption{Same-site CSRM common-input benchmark. (a) Distribution of $V_S$ MAE relative to the CSRM reference profile for QEDisp fundamental-mode multistart inversion, DI-Strong, DI-Weak, DispFormer phase-only and DispFormer phase+group. The first four methods use the same Rayleigh phase-velocity input; DispFormer phase+group uses additional group-velocity input and is shaded separately. (b) Fraction of CSRM reference-profile samples inside the DI p05--p95 posterior band. This is a reference-profile inclusion diagnostic, not empirical field-posterior coverage.}
\figalt{Two-panel benchmark figure showing box-and-strip distributions of shear-velocity error for QEDisp, DI-Strong, DI-Weak and DispFormer on the same CSRM sites, plus CSRM reference-profile inclusion for the two DI samplers.}
...
Our emphasis is on the audit of the learned posterior. We treat it as a conditional distribution defined by a synthetic prior-predictive experiment, and then ask how that distribution should be checked. The matched DI-Strong/DI-Weak comparison tests prior support directly; the reliability curves, depth-binned coverage, rank diagnostics and posterior-spread calibration test calibration; the missing-band and noise experiments test robustness to common observation changes; and the QEDisp/DispFormer comparison keeps point-estimate performance in view. This framing is deliberately narrower than recent pretrained or variable-period approaches such as DispFormer, but it addresses a different need: making learned posterior samples and intervals auditable before they are used as geophysical uncertainty.

The reduced MCMC anchor (Section~\ref{sec:mcmc_anchor_results}; Appendix Fig.~\ref{fig:mcmcanchorinside}) gives a small conventional-sampling check on this interpretation. It is not an exact full-dimensional Bayesian solution for the procedural generator, but it tests whether the learned sampler gives similar medians and intervals when the parameterisation, prior approximation, forward solver, mask and error scale are matched as closely as practical.

\subsection{Meaning of the learned posterior}
...
The current sampler is therefore a controlled probabilistic inversion framework, not a general-purpose model for arbitrary period ranges. Future work should extend the same posterior-audit philosophy to variable-period inputs, broader wave-type combinations, field-specific error models, matched-period field calibration data and independently justified geological constraints.
...
The code, synthetic-data generators, configs, random seeds, plotting scripts, evaluation CSV/JSON outputs and manuscript build commands used for this version are available in the project repository at \url{https://github.com/cangyeone/SurfFlow}. A manuscript-specific archive manifest in that repository lists the exact source files, figure PDFs, result tables, protocol files, checkpoint metadata and large artifacts required for the final DOI archive.

The CSRM same-site benchmark uses the OpenSWI-real data product \citep{LiuEtAl2026OpenSWI}. External baseline calculations use QEDispInv \citep{PanEtAl2026QEDispInv} and DispFormer \citep{LiuEtAl2025DispFormer}; the wrapper scripts and extracted benchmark tables used for the comparisons are archived with the SurfFlow result files. Checkpoint identifiers, training metadata and exact evaluation commands are recorded in the repository README files and result-table protocols. Because GitHub is not used here for large binary files, model weights and large diagnostic arrays need to be supplied through the DOI archive for final submission, or regenerated from the recorded training commands.
...
\subsection{Missing-band and noise sensitivity diagnostics}
...
\figalt{Line plots showing inside-support shear-velocity error, coverage, posterior standard deviation and spread multiplier as additive phase-velocity noise increases from zero to 0.10 km per second.}
\label{fig:noise}
\end{figure}
