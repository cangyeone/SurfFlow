...
As a small posterior anchor, Appendix Fig.~\ref{fig:mcmcanchorinside} compares DI-Strong with a reduced same-prior MCMC reference on synthetic cases. The comparison is not an exact full-dimensional Bayesian solution for the procedural generator, but it checks whether the learned sampler gives similar medians and intervals when the prior, forward solver, mask and error scale are matched as closely as practical.
...
\caption{Reduced same-prior MCMC anchor across prior-support regimes. The inside-support case shows close DI--MCMC agreement. The near-edge and outside-support cases show larger differences between the learned posterior and the reduced MCMC reference, even when the posterior-predictive fit remains acceptable. This is why the posterior is interpreted together with prior-support, coverage and posterior-predictive diagnostics rather than as an unchecked Bayesian probability.}
